...
export default function Now(currentlyReading) {
  ...
  var myBirthDate = '2000-05-01'
  var birthDate = new Date(myBirthDate)

  var years = year - birthDate.getFullYear()

  var months = 0
  if (month >= birthDate.getMonth()) months = month - birthDate.getMonth()
  else {
    years--
    months = 12 + month - birthDate.getMonth()
  }

  var days = 0
  if (date >= birthDate.getDate()) days = date - birthDate.getDate()
  else {
    months--
    days = 31 + date - birthDate.getDate()
    if (months < 0) {
      months = 11
      years--
    }
